...
def parcelx(prof, flag, *args):
    ...
    for i in iter_ranges:
        ...
        if pcl.lclhght < 6000.:
            if interp.to_agl(prof, h1) <=6000. and interp.to_agl(prof, h2) >= 6000.:
                pe3 = pelast
                h3 = interp.hght(prof, pe3)
                te3 = interp.vtmp(prof, pe3)
                tp3 = thermo.wetlift3(pe1, tp1, pe3)
        ...
        if lyre >= 0. and lyrlast <= 0.:
            ...
            pe2 = pe1
            pe3 = pelast
            if interp.vtmp(prof, pe3) < thermo.virtemp(pe3, thermo.wetlift3(pe2, tp3, pe3), thermo.wetlift3(pe2, tp3, pe3)):
        ...
        if lyre <= 0. and lyrlast >= 0.:
            ...
            pe2 = pe1
            pe3 = pelast
            while interp.vtmp(prof, pe3) < thermo.virtemp(pe3, thermo.wetlift3(pe2, tp3, pe3), thermo.wetlift3(pe2, tp3, pe3)):
                pe3 -= 5
            #pcl.elpres = pe3
...
class Parcel(object):
    def __init__(self):
        ...
        self.tmpc = -99. # Parcel beginning temperature (C)
        self.dwpc = -99. # Parcel beginning dewpoint (C)
        self.ptrace = -99. # Parcel trace pressure (mb)
        self.ttrace = -99. # Parcel trace temperature (C)
        self.blayer = -99. # Pressure of the bottom of the layer the parcel is lifted (mb)
        self.tlayer = -99. # Pressure of the top of the layer the parcel is lifted (mb)
        self.entrain = 0. # A parcel entrainment setting (not yet implemented)
        self.lclpres = -99. # Parcel LCL (lifted condensation level) pressure (mb)
        self.lclhght = -99. # Parcel LCL height (m AGL)
        self.lfcpres = -99. # Parcel LFC (level of free convection) pressure (mb)
        self.lfchght = -99. # Parcel LFC height (m AGL)
        self.elpres = -99. # Parcel EL (equilibrium level) pressure (mb)
        self.elhght = -99. # Parcel EL height (m AGL)
        self.mplpres = -99. # Maximum Parcel Level (mb)
        self.mplhght = -99. # Maximum Parcel Level (m AGL)
        self.bplus = -99. # Parcel CAPE (J/kg)
        self.bminus = -99. # Parcel CIN (J/kg)
        self.pbot = -99.
        self.ptop =- 99.
        self.lplhght = -99. # Lifted Parcel Height (m AGL)
        '''
        ...
        if mucape >= ecape and mucinh > ecinh:
            ...
            for i in range(bptr+1, prof.top):
                ...
                if pcl.bplus < ecape or pcl.bminus <= ecinh: #Is this a potential "top"?
                    j = 1
                    if (prof.dwpc[i-j] <-999) and (prof.tmpc[i-j] < -999.):
                        j += 1
                    ptop = prof.pres[i-j]
...
def cape(prof, pres, tmpc, dwpc):
    ...
    effective_inflow_layer. This was done to get around the lack of **kwarg
    support in numba jit.
    '''
